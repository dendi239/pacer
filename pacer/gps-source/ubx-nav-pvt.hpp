...
// UBX-NAV-PVT wire structure, shared between the desktop .dat reader
// (gps-source-dat.cpp) and the ESP32 firmware's UART parser/logger.
// Field naming and types follow the u-blox interface manual; layout matches
// the raw 92-byte message payload, so it can be memcpy'd straight out of a
// UBX frame and fwrite'n to a .dat log.
